...
impl Messgroesse {
    ...
    /// assert_eq!(Messgroesse::iter_known().count(), Messgroesse::COUNT);
    /// assert!(Messgroesse::iter_known().all(|v| v.is_known()));
    /// ```
...
/// define becomes [`Messgroesse::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Messgroesse::from_wire`] on a `String` column, or check
/// [`Messgroesse::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
