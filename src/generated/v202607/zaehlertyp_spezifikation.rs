...
impl ZaehlertypSpezifikation {
    ...
    /// # Example
    /// ```
    /// # use rubo4e::current::ZaehlertypSpezifikation;
    /// // Never yields the `Unknown` catch-all, so the count matches `COUNT`.
    /// assert_eq!(ZaehlertypSpezifikation::iter_known().count(), ZaehlertypSpezifikation::COUNT);
    /// assert!(ZaehlertypSpezifikation::iter_known().all(|v| v.is_known()));
    /// ```
    ...
    /// ```
    /// # use rubo4e::current::ZaehlertypSpezifikation;
    /// assert_eq!(ZaehlertypSpezifikation::from_wire("EDL40"), Ok(ZaehlertypSpezifikation::Edl40));
    ...
    /// assert!(ZaehlertypSpezifikation::from_wire("UNKNOWN").is_err());
    /// ```
...
/// define becomes [`ZaehlertypSpezifikation::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`ZaehlertypSpezifikation::from_wire`] on a `String` column, or check
/// [`ZaehlertypSpezifikation::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
...
/// Lets `Vec<ZaehlertypSpezifikation>` bind to a `TEXT[]` column.  Only this crate can
/// provide it: the trait and the enum are both foreign to any consumer, so the
/// orphan rule rules out a downstream impl.
#[cfg(feature = "sqlx")]
impl sqlx::postgres::PgHasArrayType for ZaehlertypSpezifikation {
