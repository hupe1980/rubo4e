...
impl Messwertstatus {
    ...
    /// assert_eq!(Messwertstatus::iter_known().count(), Messwertstatus::COUNT);
    /// assert!(Messwertstatus::iter_known().all(|v| v.is_known()));
    /// ```
...
/// define becomes [`Messwertstatus::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Messwertstatus::from_wire`] on a `String` column, or check
/// [`Messwertstatus::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
...
/// Lets `Vec<Messwertstatus>` bind to a `TEXT[]` column.  Only this crate can
/// provide it: the trait and the enum are both foreign to any consumer, so the
/// orphan rule rules out a downstream impl.
#[cfg(feature = "sqlx")]
impl sqlx::postgres::PgHasArrayType for Messwertstatus {
