...
impl Oekolabel {
    ...
    /// # Example
    /// ```
    /// # use rubo4e::current::Oekolabel;
    /// // Never yields the `Unknown` catch-all, so the count matches `COUNT`.
    /// assert_eq!(Oekolabel::iter_known().count(), Oekolabel::COUNT);
    /// assert!(Oekolabel::iter_known().all(|v| v.is_known()));
    /// ```
...
/// define becomes [`Oekolabel::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Oekolabel::from_wire`] on a `String` column, or check
/// [`Oekolabel::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
