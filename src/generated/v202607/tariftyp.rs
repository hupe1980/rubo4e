...
impl Tariftyp {
    ...
    /// # Example
    /// ```
    /// # use rubo4e::current::Tariftyp;
    /// // Never yields the `Unknown` catch-all, so the count matches `COUNT`.
    /// assert_eq!(Tariftyp::iter_known().count(), Tariftyp::COUNT);
    /// assert!(Tariftyp::iter_known().all(|v| v.is_known()));
    /// ```
    ...
    /// ```
    /// # use rubo4e::current::Tariftyp;
    /// assert_eq!(Tariftyp::from_wire("GRUND_ERSATZVERSORGUNG"), Ok(Tariftyp::GrundErsatzversorgung));
    /// // Out-of-schema values are rejected rather than degraded:
    /// assert!(Tariftyp::from_wire("NOT_A_REAL_VALUE").is_err());
    /// // …including the `Unknown` catch-all's own wire spelling:
    /// assert!(Tariftyp::from_wire("UNKNOWN").is_err());
    /// ```
...
/// define becomes [`Tariftyp::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Tariftyp::from_wire`] on a `String` column, or check
/// [`Tariftyp::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
