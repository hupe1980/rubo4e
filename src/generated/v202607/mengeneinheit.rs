...
impl Mengeneinheit {
    ...
    /// assert_eq!(Mengeneinheit::iter_known().count(), Mengeneinheit::COUNT);
    /// assert!(Mengeneinheit::iter_known().all(|v| v.is_known()));
    /// ```
...
/// define becomes [`Mengeneinheit::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Mengeneinheit::from_wire`] on a `String` column, or check
/// [`Mengeneinheit::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
