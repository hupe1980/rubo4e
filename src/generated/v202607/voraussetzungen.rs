...
impl Voraussetzungen {
    ...
    /// # Example
    /// ```
    /// # use rubo4e::current::Voraussetzungen;
    /// // Never yields the `Unknown` catch-all, so the count matches `COUNT`.
    /// assert_eq!(Voraussetzungen::iter_known().count(), Voraussetzungen::COUNT);
    /// assert!(Voraussetzungen::iter_known().all(|v| v.is_known()));
    /// ```
    ...
    /// ```
    /// # use rubo4e::current::Voraussetzungen;
    /// assert_eq!(Voraussetzungen::from_wire("EINZUGSERMAECHTIGUNG"), Ok(Voraussetzungen::Einzugsermaechtigung));
    /// // Out-of-schema values are rejected rather than degraded:
    /// assert!(Voraussetzungen::from_wire("NOT_A_REAL_VALUE").is_err());
    /// // …including the `Unknown` catch-all's own wire spelling:
    /// assert!(Voraussetzungen::from_wire("UNKNOWN").is_err());
    /// ```
...
/// define becomes [`Voraussetzungen::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Voraussetzungen::from_wire`] on a `String` column, or check
/// [`Voraussetzungen::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
...
/// Lets `Vec<Voraussetzungen>` bind to a `TEXT[]` column.  Only this crate can
/// provide it: the trait and the enum are both foreign to any consumer, so the
/// orphan rule rules out a downstream impl.
#[cfg(feature = "sqlx")]
impl sqlx::postgres::PgHasArrayType for Voraussetzungen {
