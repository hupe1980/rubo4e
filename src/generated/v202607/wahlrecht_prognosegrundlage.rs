...
impl WahlrechtPrognosegrundlage {
    ...
    ///
    /// Excludes the forward-compatibility [`WahlrechtPrognosegrundlage::Unknown`] catch-all, so this
    ...
    /// [`WahlrechtPrognosegrundlage::Unknown`] catch-all.  Stable for this schema version.
    pub const COUNT: usize = Self::VARIANTS.len();
    /// Returns an iterator over all **known** variants of `WahlrechtPrognosegrundlage`.
    ///
    /// Yields only variants that correspond to values defined in the BO4E schema
    /// (i.e. [`Self::VARIANTS`]), never the [`WahlrechtPrognosegrundlage::Unknown`] catch-all.
    /// Available **without** the `strum` feature.
    ///
    /// # Example
    /// ```
    /// # use rubo4e::current::WahlrechtPrognosegrundlage;
    /// // Never yields the `Unknown` catch-all, so the count matches `COUNT`.
    /// assert_eq!(WahlrechtPrognosegrundlage::iter_known().count(), WahlrechtPrognosegrundlage::COUNT);
    /// assert!(WahlrechtPrognosegrundlage::iter_known().all(|v| v.is_known()));
    /// ```
    ...
    ///
    /// [`WahlrechtPrognosegrundlage::Unknown`] renders as `"UNKNOWN"`, matching its serialized form.
    ...
    /// to [`WahlrechtPrognosegrundlage::Unknown`] — this returns
    ...
    /// ```
    /// # use rubo4e::current::WahlrechtPrognosegrundlage;
    /// assert_eq!(WahlrechtPrognosegrundlage::from_wire("DURCH_LF"), Ok(WahlrechtPrognosegrundlage::DurchLf));
    /// // Out-of-schema values are rejected rather than degraded:
    /// assert!(WahlrechtPrognosegrundlage::from_wire("NOT_A_REAL_VALUE").is_err());
    /// // …including the `Unknown` catch-all's own wire spelling:
    /// assert!(WahlrechtPrognosegrundlage::from_wire("UNKNOWN").is_err());
    /// ```
    ...
    /// [`WahlrechtPrognosegrundlage::Unknown`] catch-all (an out-of-schema value).
...
/// define becomes [`WahlrechtPrognosegrundlage::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`WahlrechtPrognosegrundlage::from_wire`] on a `String` column, or check
/// [`WahlrechtPrognosegrundlage::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
...
/// Lets `Vec<WahlrechtPrognosegrundlage>` bind to a `TEXT[]` column.  Only this crate can
/// provide it: the trait and the enum are both foreign to any consumer, so the
/// orphan rule rules out a downstream impl.
#[cfg(feature = "sqlx")]
impl sqlx::postgres::PgHasArrayType for WahlrechtPrognosegrundlage {
