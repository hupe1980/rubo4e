...
impl Preistyp {
    ...
    /// # Example
    /// ```
    /// # use rubo4e::current::Preistyp;
    /// // Never yields the `Unknown` catch-all, so the count matches `COUNT`.
    /// assert_eq!(Preistyp::iter_known().count(), Preistyp::COUNT);
    /// assert!(Preistyp::iter_known().all(|v| v.is_known()));
    /// ```
...
/// define becomes [`Preistyp::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Preistyp::from_wire`] on a `String` column, or check
/// [`Preistyp::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
