...
impl AbgabeArt {
    ...
    ///
    /// Excludes the forward-compatibility [`AbgabeArt::Unknown`] catch-all, so this
    ...
    /// [`AbgabeArt::Unknown`] catch-all.  Stable for this schema version.
    pub const COUNT: usize = Self::VARIANTS.len();
    /// Returns an iterator over all **known** variants of `AbgabeArt`.
    ///
    /// Yields only variants that correspond to values defined in the BO4E schema
    /// (i.e. [`Self::VARIANTS`]), never the [`AbgabeArt::Unknown`] catch-all.
    /// Available **without** the `strum` feature.
    ///
    /// # Example
    /// ```
    /// # use rubo4e::current::AbgabeArt;
    /// // Never yields the `Unknown` catch-all, so the count matches `COUNT`.
    /// assert_eq!(AbgabeArt::iter_known().count(), AbgabeArt::COUNT);
    /// assert!(AbgabeArt::iter_known().all(|v| v.is_known()));
    /// ```
    ...
    ///
    /// [`AbgabeArt::Unknown`] renders as `"UNKNOWN"`, matching its serialized form.
    ...
    /// to [`AbgabeArt::Unknown`] — this returns
    ...
    /// ```
    /// # use rubo4e::current::AbgabeArt;
    /// assert_eq!(AbgabeArt::from_wire("KAS"), Ok(AbgabeArt::Kas));
    /// // Out-of-schema values are rejected rather than degraded:
    /// assert!(AbgabeArt::from_wire("NOT_A_REAL_VALUE").is_err());
    /// // …including the `Unknown` catch-all's own wire spelling:
    /// assert!(AbgabeArt::from_wire("UNKNOWN").is_err());
    /// ```
    ...
    /// [`AbgabeArt::Unknown`] catch-all (an out-of-schema value).
...
impl std::fmt::Display for AbgabeArt {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(self.as_wire())
    }
}
impl AsRef<str> for AbgabeArt {
    fn as_ref(&self) -> &str {
        self.as_wire()
    }
}
#[cfg(feature = "versioned")]
impl crate::bo4e_enum_sealed::Sealed for AbgabeArt {}
#[cfg(feature = "versioned")]
impl crate::Bo4eEnum for AbgabeArt {
...
#[cfg(feature = "versioned")]
impl crate::Bo4eStrict for AbgabeArt {
...
/// define becomes [`AbgabeArt::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`AbgabeArt::from_wire`] on a `String` column, or check
/// [`AbgabeArt::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
