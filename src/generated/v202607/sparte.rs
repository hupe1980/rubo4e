...
impl Sparte {
    ...
    /// assert_eq!(Sparte::iter_known().count(), Sparte::COUNT);
    /// assert!(Sparte::iter_known().all(|v| v.is_known()));
    /// ```
    ...
    /// assert!(Sparte::from_wire("UNKNOWN").is_err());
    /// ```
...
/// define becomes [`Sparte::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Sparte::from_wire`] on a `String` column, or check
/// [`Sparte::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
