...
impl Organisationstyp {
    ...
    /// # Example
    /// ```
    /// # use rubo4e::current::Organisationstyp;
    /// // Never yields the `Unknown` catch-all, so the count matches `COUNT`.
    /// assert_eq!(Organisationstyp::iter_known().count(), Organisationstyp::COUNT);
    /// assert!(Organisationstyp::iter_known().all(|v| v.is_known()));
    /// ```
    ...
    /// ```
    /// # use rubo4e::current::Organisationstyp;
    /// assert_eq!(Organisationstyp::from_wire("PRIVATPERSON"), Ok(Organisationstyp::Privatperson));
    ...
    /// assert!(Organisationstyp::from_wire("UNKNOWN").is_err());
    /// ```
...
/// define becomes [`Organisationstyp::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Organisationstyp::from_wire`] on a `String` column, or check
/// [`Organisationstyp::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
...
/// Lets `Vec<Organisationstyp>` bind to a `TEXT[]` column.  Only this crate can
/// provide it: the trait and the enum are both foreign to any consumer, so the
/// orphan rule rules out a downstream impl.
#[cfg(feature = "sqlx")]
impl sqlx::postgres::PgHasArrayType for Organisationstyp {
