...
impl Erzeugungsart {
    ...
    /// # Example
    /// ```
    /// # use rubo4e::current::Erzeugungsart;
    /// // Never yields the `Unknown` catch-all, so the count matches `COUNT`.
    /// assert_eq!(Erzeugungsart::iter_known().count(), Erzeugungsart::COUNT);
    /// assert!(Erzeugungsart::iter_known().all(|v| v.is_known()));
    /// ```
...
/// define becomes [`Erzeugungsart::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Erzeugungsart::from_wire`] on a `String` column, or check
/// [`Erzeugungsart::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
...
/// Lets `Vec<Erzeugungsart>` bind to a `TEXT[]` column.  Only this crate can
/// provide it: the trait and the enum are both foreign to any consumer, so the
/// orphan rule rules out a downstream impl.
#[cfg(feature = "sqlx")]
impl sqlx::postgres::PgHasArrayType for Erzeugungsart {
