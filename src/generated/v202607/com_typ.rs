...
impl ComTyp {
    ...
    /// assert_eq!(ComTyp::iter_known().count(), ComTyp::COUNT);
    /// assert!(ComTyp::iter_known().all(|v| v.is_known()));
    /// ```
...
/// define becomes [`ComTyp::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`ComTyp::from_wire`] on a `String` column, or check
/// [`ComTyp::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
