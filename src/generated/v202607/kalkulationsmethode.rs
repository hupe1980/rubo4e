...
impl Kalkulationsmethode {
    ...
    /// # Example
    /// ```
    /// # use rubo4e::current::Kalkulationsmethode;
    /// // Never yields the `Unknown` catch-all, so the count matches `COUNT`.
    /// assert_eq!(Kalkulationsmethode::iter_known().count(), Kalkulationsmethode::COUNT);
    /// assert!(Kalkulationsmethode::iter_known().all(|v| v.is_known()));
    /// ```
    ...
    /// ```
    /// # use rubo4e::current::Kalkulationsmethode;
    /// assert_eq!(Kalkulationsmethode::from_wire("STUFEN"), Ok(Kalkulationsmethode::Stufen));
    ...
    /// assert!(Kalkulationsmethode::from_wire("UNKNOWN").is_err());
    /// ```
...
/// define becomes [`Kalkulationsmethode::Unknown`] rather than a decode error, so a
/// database row written by a newer schema version still reads back.
///
/// Use [`Kalkulationsmethode::from_wire`] on a `String` column, or check
/// [`Kalkulationsmethode::is_known`], where out-of-schema values must be rejected.
#[cfg(feature = "sqlx")]
...
/// Lets `Vec<Kalkulationsmethode>` bind to a `TEXT[]` column.  Only this crate can
/// provide it: the trait and the enum are both foreign to any consumer, so the
/// orphan rule rules out a downstream impl.
#[cfg(feature = "sqlx")]
impl sqlx::postgres::PgHasArrayType for Kalkulationsmethode {
