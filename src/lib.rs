...
/// BO4E schema v202601 types — **placeholder, pending schema publication**.
///
/// The upstream BO4E v202601 schema (BDEW FV2026-10-01) has not yet been
/// published.  Until `just generate v202601.0.0` produces a real generated
/// module, this re-exports all `v202501` types unchanged.
///
/// **Type-identity caveat:** while this placeholder is active,
/// `v202601::Rechnung` and `v202501::Rechnung` are literally the same Rust
/// type, so the compiler cannot catch accidental cross-version mixing.
/// Code written against `use rubo4e::v202601 as bo4e` will compile without
/// changes once the real schema ships — but may need updates for new fields.
///
/// Use `rubo4e::v202501` to pin to the stable, real schema today.
/// Use `rubo4e::current` to always track the latest *real* release.
///
/// **To replace with the real module once the schema is available:**
/// ```text
/// just download-schemas v202601.0.0
/// just generate v202601.0.0
/// ```
/// Then remove this inline module from `src/lib.rs` — the generator will
/// have written `src/generated/v202601/` and the `pub mod v202601` block
/// below should be replaced with:
/// ```rust,ignore
/// pub mod v202601 { pub use crate::generated::v202601::*; }
/// ```
#[cfg(feature = "versioned")]
#[cfg_attr(docsrs, doc(cfg(feature = "versioned")))]
pub mod v202601 {
    pub use crate::v202501::*;
}

/// Alias to the latest **real** (non-placeholder) BO4E schema version.
///
/// Currently resolves to `v202501`.  Will be updated to `v202601` once the
/// upstream BO4E v202601 schema is generated by `just generate v202601.0.0`.
///
/// Use `rubo4e::current` when you always want the newest stable types and do
/// not need to pin to a specific version.  Pin to a concrete versioned module
/// (`rubo4e::v202501`) for compile-time stability across crate updates.
#[cfg(feature = "versioned")]
